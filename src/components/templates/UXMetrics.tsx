...
export const UXMetrics: React.FC = () => {
    ...
    <div className="h-full w-full bg-gray-50 overflow-auto">
      ...
      <div className="max-w-6xl mx-auto p-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold">UX Metrics</h2>
          <button
                    ...
                    <div key={metric.id} className="bg-gray-50 p-4 rounded-lg">
                      ...
                      <div className="mb-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <button
                            onClick={() => handleAddArrayItem(group.id, metric.id, 'insights')}
                          ...
                          {metric.insights.map((insight, index) => (
                            <div key={index} className="flex gap-2">
                              <input
                                type="text"
                                value={insight}
                                onChange={(e) => handleUpdateArrayItem(group.id, metric.id, 'insights', index, e.target.value)}
                                className="flex-1 p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                placeholder="Add insight..."
                              />
                              <button
                                onClick={() => handleRemoveArrayItem(group.id, metric.id, 'insights', index)}
                      ...
                      <div className="mb-4">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <button
                            onClick={() => handleAddArrayItem(group.id, metric.id, 'actions')}
                            className="text-indigo-600 hover:text-indigo-700 text-sm"
                          >
                            + Add Action
                          </button>
                          ...
                          {metric.actions.map((action, index) => (
                            <div key={index} className="flex gap-2">
                              <input
                                type="text"
                                value={action}
                                onChange={(e) => handleUpdateArrayItem(group.id, metric.id, 'actions', index, e.target.value)}
                                className="flex-1 p-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                                placeholder="Add action..."
                              />
                              <button
                                onClick={() => handleRemoveArrayItem(group.id, metric.id, 'actions', index)}
                                className="text-red-500 hover:text-red-700"
                              >
                                <X className="w-4 h-4" />
                              </button>
